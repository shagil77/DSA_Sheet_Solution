...
class Solution{
    int findPartIdx(int pre[], int l, int h, int node) {
        for(int i=l; i<=h; i++) {
            if(pre[i]>node) return (i-1);
        }
        
        return (h+1);
    }
    Node* solve(int pre[], int l, int h) {
        if(l>h) return NULL;
        
        Node* root = newNode(pre[l]);
        int idx = findPartIdx(pre, l+1, h, pre[l]);
        
        if(idx<=l) {
            root->left = NULL;
            root->right = solve(pre, l+1, h);
        } else if(idx>=h) {
            root->right = NULL;
            root->left = solve(pre, l+1, h);
        } else {
            root->left = solve(pre, l+1, idx);
            root->right = solve(pre, idx+1, h);
        }
